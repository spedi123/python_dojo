...
function rotateStr(str, amnt) {
  ...
  newStr1 = str.slice(0, str.length - amnt);
  newStr2 = str.slice(str.length - amnt, str.length);
...
function isRotation(s1, s2) {
  ...
  for (var i = 1; i < s1.length; i++) {
    newStr1 = s1.slice(0, s1.length - i);
    newStr2 = s1.slice(s1.length - i, s1.length);
    if (newStr2 + newStr1 == s2) {
      return true;
    }
  }
  return false;
